...
void main() {
  ...
  test(
    ...
    expect(SystemVerilogService(mod).synthOutput, contains('shouldExist'));
  });

  test('unconnected port does not duplicate internal signal', () async {
    final pMod = ParentMod(Logic(), Logic());
    await pMod.build();
    final sv = SystemVerilogService(pMod).synthOutput;
  ...
  test('badly named intermediate signal sanitization', () async {
    ...
    final sv = SystemVerilogService(dut).synthOutput;
