...
class FilterChannel extends Module {
  ...
  FilterChannel(
    ...
    Combinational([
      If(
        atFirstTap,
        then: [accumFeedback < Const(0, width: dataWidth)],
        orElse: [accumFeedback < accumReg],
      ),
    ]);
    ...
    Sequential(clk, reset: reset, [
      If(
        ...
        then: [
          If(
            lastTap,
            then: [tapCounter < Const(0, width: tapIdxWidth)],
    ...
    Sequential(clk, reset: reset, [
      lastTapD1 < lastTap,
      lastTapD2 < lastTapD1,
      If(lastTapD2, then: [outputReg < accumReg]),
    ]);
    ...
    Sequential(clk, reset: reset, [
      If(enable, then: [validPipe < outputReady]),
    ]);
    ...
    Combinational([
      If(
        validPipe,
        then: [dataOut < outputReg],
        orElse: [dataOut < Const(0, width: dataWidth)],
      ),
      validOut < validPipe,
    ]);
  }
