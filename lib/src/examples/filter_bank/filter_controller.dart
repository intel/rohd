...
class FilterController extends Module {
  ...
  FilterController(
    ...
    Logic inputValid,
    Logic inputDone, {
    required int drainCycles,
    super.name = 'FilterController',
    ...
    _fsm = FiniteStateMachine<FilterState>(clk, reset, FilterState.idle, [
      State<FilterState>(
        FilterState.idle,
        events: {start: FilterState.loading},
        actions: [filterEnable < 0, loadingPhase < 0, doneFlag < 0],
      ),
      State<FilterState>(
        FilterState.loading,
        events: {inputValid: FilterState.running},
        actions: [filterEnable < 1, loadingPhase < 1, doneFlag < 0],
      ),
      State<FilterState>(
        FilterState.running,
        events: {inputDone: FilterState.draining},
        actions: [filterEnable < 1, loadingPhase < 0, doneFlag < 0],
      ),
      State<FilterState>(
        FilterState.draining,
        events: {drainDone: FilterState.done},
        actions: [filterEnable < 1, loadingPhase < 0, doneFlag < 0],
      ),
      State<FilterState>(
        FilterState.done,
        events: {},
        actions: [filterEnable < 0, loadingPhase < 0, doneFlag < 1],
      ),
    ]);
    ...
    Sequential(clk, reset: reset, [
      If(
        isDraining,
        then: [drainCount < drainCount + Const(1, width: 8)],
        orElse: [drainCount < Const(0, width: 8)],
      ),
    ]);
  }
}
