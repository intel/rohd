...
class NetlistPasses {
  ...
  static void applyMaximalSubsetGrouping(
    ...
    for (final moduleDef in allModules.values) {
      ...
      for (final concatEntry in cells.entries.toList()) {
        ...
        for (final inputBit in inputBits) {
          final (traced, chain) =
              NetlistUtils.traceBackward(inputBit, wireDriverCell, cells);
        ...
        if (yWidth == aWidth) {
          cellsToAdd['maxsub_buf_$replIdx'] = NetlistUtils.makeBufCell(
              aWidth, sourceBusParentBits, outputBits.cast<Object>());
        } else {
          cellsToAdd['maxsub_slice_$replIdx'] = NetlistUtils.makeSliceCell(
              offset,
              aWidth,
              yWidth,
              sourceBusParentBits,
              outputBits.cast<Object>());
        }
        replIdx++;
      }
  ...
  static void applyStructConversionGrouping(
    Map<String, Map<String, Object?>> allModules,
  ) {
  ...
  static void applyStructBufferInsertion(
    Map<String, Map<String, Object?>> allModules,
  ) {
    for (final moduleName in allModules.keys.toList()) {
      final moduleDef = allModules[moduleName]!;
      ...
      for (final sliceGroup in slicesByInput.values) {
        ...
        final modPorts =
            moduleDef['ports'] as Map<String, Map<String, Object?>>?;
        ...
        // Insert $buf cell.
        cells['struct_buf_$bufIdx'] =
            NetlistUtils.makeBufCell(aWidth, aBus, yBus);
        bufIdx++;
      }
    }
  }
  ...
  static void collapseStructGroupModules(
    Map<String, Map<String, Object?>> allModules,
  ) {
    ...
    for (final moduleDef in allModules.values) {
      ...
      for (final entry in cells.entries) {
        ...
        for (final pEntry in synthPorts.entries) {
          ...
          final localBits =
              (pEntry.value['bits'] as List?)?.cast<Object>() ?? [];
            ...
            if (cc.containsKey('A')) {
              ...
              for (final portName in cc.keys) {
                ...
                if (m != null) {
                  ...
                  final lo = m.group(2) != null ? int.parse(m.group(2)!) : hi;
                  rangePorts[lo] =
                      (cc[portName] as List?)?.cast<Object>() ?? [];
                }
              }
        ...
        for (final pEntry in synthPorts.entries) {
          ...
          if (sourcePortNames.length == 1) {
            ...
            if (portOutputBits.length == srcWidth) {
              // Same width → $buf
              perPortCells['${cellKey}_buf'] = NetlistUtils.makeBufCell(
              ...
              perPortCells['${cellKey}_slice'] = NetlistUtils.makeSliceCell(
                  offset,
                  srcWidth,
  ...
  static void applyConcatToBufferReplacement(
    Map<String, Map<String, Object?>> allModules,
  ) {
    for (final moduleDef in allModules.values) {
      final cells = moduleDef['cells'] as Map<String, Map<String, Object?>>?;
      ...
      for (final entry in cells.entries) {
        ...
        for (final pe in conns.entries) {
          ...
          final match = NetlistUtils.rangePortRe.firstMatch(pe.key);
          if (match == null) {
        ...
        additions['concat_buf_$bufIdx'] =
            NetlistUtils.makeBufCell(allInputBits.length, allInputBits, yBits);
        bufIdx++;
      }
  ...
  static void applyCollapseSelectsIntoPack(
    Map<String, Map<String, Object?>> allModules,
  ) {
    for (final moduleDef in allModules.values) {
      final cells = moduleDef['cells'] as Map<String, Map<String, Object?>>?;
  ...
  static void applyCollapseUnpackToConcat(
    ...
    for (final moduleEntry in allModules.entries) {
      ...
      while (anyChanged && iteration < 20) {
        ...
        for (final concatEntry in cells.entries.toList()) {
          ...
          if (runs.isEmpty) {
            ...
            cellsToRemove
              ...
              ..add(uName)
              ..add(concatName);
            cellsToAdd['unpack_concat_buf_$replIdx'] = NetlistUtils.makeBufCell(
                reorderedA.length, reorderedA, outputBits);
            replIdx++;
            continue;
          }
          ...
          if (allCollapsed) {
            ...
            if (yWidth == aWidth) {
              cellsToAdd['unpack_concat_buf_$replIdx'] =
                  NetlistUtils.makeBufCell(aWidth, sourceBits, outputBits);
            } else {
              cellsToAdd['unpack_concat_buf_$replIdx'] =
                  NetlistUtils.makeSliceCell(
          ...
          while (portIdx < inputPorts.length) {
            ...
            if (activeRun != null) {
              ...
              final uParams = cells[t0.unpackName!]!['parameters']
        ...
        final wireMaps2 = NetlistUtils.buildWireMaps(cells, moduleDef);
  ...
  static void applyCollapseUnpackToPack(
    Map<String, Map<String, Object?>> allModules,
  ) {
    for (final moduleDef in allModules.values) {
      final cells = moduleDef['cells'] as Map<String, Map<String, Object?>>?;
