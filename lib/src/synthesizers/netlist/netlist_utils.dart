...
String? portNameForSynthLogic(SynthLogic sl, Map<String, Logic> portMap) {
...
/// been replaced).
String? tryGetSynthLogicName(SynthLogic sl) {
  try {
    return sl.name;
...
SynthLogic resolveReplacement(SynthLogic sl) {
  var r = sl;
...
final rangePortRe = RegExp(r'^\[(\d+)(?::(\d+))?\]$');

/// Create a `$buf` cell map.
Map<String, Object?> makeBufCell(
  int width,
  List<Object> aBits,
  List<Object> yBits,
) =>
...
Map<String, Object?> makeSliceCell(
  int offset,
  int aWidth,
  int yWidth,
  List<Object> aBits,
...
(int sourceBit, Set<String> intermediates) traceBackward(
  int startBit,
  Map<int, String> wireDriverCell,
  Map<String, Map<String, Object?>> cells,
) {
...
bool isConstantSynthLogic(SynthLogic sl) => resolveReplacement(sl).isConstant;

/// Extract the Const value from a constant SynthLogic.
Const? constValueFromSynthLogic(SynthLogic sl) {
