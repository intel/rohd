...
class SynchronousSubscription<T> {
  ...
  // ignore: unsafe_variance - only called with T, private
  final void Function(T args) _func;

  /// If true, then this subscription is actively triggering [_func] on the
  /// registered event.
  ...
  /// Constructs a new subscription so that [_func] executes on certain events.
  SynchronousSubscription(this._func);

  /// Cancels the subscription, so that [_func] will no longer be called when
