...
/// Efficient implementation of a set-like datastructure that also has fast index access
///
/// 2021 July 13
...
///
/// This collection stores all data twice: once in a [Set] and once in a [List].  For
/// index access, it uses the [List].  For [contains()], it uses the [Set].  Other operations
/// like [add()] and [remove()] pay the penalty of performing the operation twice, once on
/// each collection.
///
/// In situations where it is necessary to iterate through and frequently access elements by index,
/// but also check whether a certain element is contained wihin it, and there are many elements,
/// this implementation is substantially faster than using either a [Set] or a [List].
class TraverseableCollection<T> {
  ...
  /// The method has no effect if [collection] was not in the set.
  bool remove(T item) {
