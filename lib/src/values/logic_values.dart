...
class _SmallLogicValues extends LogicValues {
  ...
  LogicValues _getRange(int start, int end) {
    ...
    return _SmallLogicValues(
      (_value >> start) & _maskOfLength(newLength),
      (_invalid >> start) & _maskOfLength(newLength),
      newLength
    );
  }
  ...
  @override
  int toInt() {
    if(_invalid != 0) throw Exception('Cannot convert invalid LogicValues to int: ${this}');
    return _value;
  }

  @override LogicValues operator ~() => _SmallLogicValues(
    ~_value & ~_invalid & _mask,
    _invalid,
    length
  );

  @override LogicValues _and2(LogicValues other) {
    if(other is! _SmallLogicValues) throw Exception('Cannot handle type ${other.runtimeType} here.');
    var eitherInvalid = _invalid | other._invalid;
    var eitherZero = (~_value & ~_invalid) | (~other._value & ~other._invalid);
    return _SmallLogicValues(
      ~eitherInvalid & ~eitherZero,
      eitherInvalid & ~eitherZero,
      length
    );
  }
  @override LogicValues _or2(LogicValues other) {
    if(other is! _SmallLogicValues) throw Exception('Cannot handle type ${other.runtimeType} here.');
    var eitherInvalid = _invalid | other._invalid;
    var eitherOne = (_value & ~_invalid) | (other._value & ~other._invalid);
    return _SmallLogicValues(
      eitherOne,
      eitherInvalid & ~eitherOne,
      length
    );
  }
  @override LogicValues _xor2(LogicValues other) {
    if(other is! _SmallLogicValues) throw Exception('Cannot handle type ${other.runtimeType} here.');
    var eitherInvalid = _invalid | other._invalid;
    return _SmallLogicValues(
      (_value ^ other._value) & ~eitherInvalid,
      eitherInvalid,
      length
    );
  }
  ...
  @override
  LogicValues _shiftLeft(int shamt) => !isValid ? _FilledLogicValues(LogicValue.x, length) :
    _SmallLogicValues(
      (_value << shamt) & _mask,
  ...
  @override
  LogicValues _shiftRight(int shamt) => !isValid ? _FilledLogicValues(LogicValue.x, length) :
    _SmallLogicValues(
      _value >> shamt,
      _invalid >> shamt,
      length
  ...
  @override
  LogicValues _shiftArithmeticRight(int shamt) => !isValid ? _FilledLogicValues(LogicValue.x, length) :
    _SmallLogicValues(
      ((_value | (this[length-1] == LogicValue.one ? ~_mask : 0)) >> shamt) & _mask,
      _invalid >> shamt,
      length
    );
...
class _BigLogicValues extends LogicValues {
  ...
  @override
  int toInt() => 
    throw Exception('LogicValues length $length is too long to convert to int. Use toBigInt() instead.');

  @override LogicValues operator ~() => _BigLogicValues(
    ~_value & ~_invalid & _mask,
    _invalid,
    length
  );
  ...
  @override LogicValues _and2(LogicValues other) {
    if(other is! _BigLogicValues) throw Exception('Cannot handle type ${other.runtimeType} here.');
    var eitherInvalid = _invalid | other._invalid;
    var eitherZero = (~_value & ~_invalid) | (~other._value & ~other._invalid);
    return _BigLogicValues(
      ~eitherInvalid & ~eitherZero,
  ...
  LogicValues _shiftArithmeticRight(int shamt) => !isValid ? _FilledLogicValues(LogicValue.x, length) :
    _BigLogicValues(
      ((_value | (this[length-1] == LogicValue.one ? ((_mask >> (length - shamt)) << (length - shamt)) : BigInt.zero)) >> shamt),
      _invalid >> shamt,
      length
    );
}
...
class _FilledLogicValues extends LogicValues {
  ...
  @override
  LogicValues _getRange(int start, int end) => _FilledLogicValues(_value, end - start);
  ...
  @override LogicValues operator ~() => _FilledLogicValues(~_value, length);

  _SmallLogicValues _toSmallLogicValues() => 
    _value == LogicValue.x ? _SmallLogicValues(0, _SmallLogicValues._maskOfLength(length), length) :
    _value == LogicValue.z ? _SmallLogicValues(_SmallLogicValues._maskOfLength(length), _SmallLogicValues._maskOfLength(length), length) :
    _value == LogicValue.one ? _SmallLogicValues(_SmallLogicValues._maskOfLength(length), 0, length) :
    _SmallLogicValues(0, 0, length);
  
  _BigLogicValues _toBigLogicValues() => 
    _value == LogicValue.x ? _BigLogicValues(BigInt.zero, _BigLogicValues._maskOfLength(length), length) :
    _value == LogicValue.z ? _BigLogicValues(_BigLogicValues._maskOfLength(length), _BigLogicValues._maskOfLength(length), length) :
    _value == LogicValue.one ? _BigLogicValues(_BigLogicValues._maskOfLength(length), BigInt.zero, length) :
  ...
  @override LogicValues _and2(LogicValues other) {
    ...
    } else if(other is _SmallLogicValues) { // _value is 1
      return _SmallLogicValues(other._value & ~other._invalid, other._invalid, length);
    } else if(other is _BigLogicValues) {   // _value is 1
      return _BigLogicValues(other._value & ~other._invalid, other._invalid, length);
    }
  ...
  @override LogicValues _or2(LogicValues other) {
    ...
    } else if(other is _SmallLogicValues) { // _value is 0
      return _SmallLogicValues(other._value & ~other._invalid, other._invalid, length);
    } else if(other is _BigLogicValues) {   // _value is 0
      return _BigLogicValues(other._value & ~other._invalid, other._invalid, length);
    }
    throw Exception('Unhandled scenario.');
  }
  ...
  @override LogicValues _xor2(LogicValues other) {
    if(other is _FilledLogicValues) {
      ...
      if(other is _SmallLogicValues) {
        return _SmallLogicValues(~other._value & other._mask & ~other._invalid, other._invalid, length);
      } else if(other is _BigLogicValues) {
        return _BigLogicValues(~other._value & other._mask & ~other._invalid, other._invalid, length);
      }
    }
    throw Exception('Unhandled scenario.');
  }
  ...
  @override
  LogicValue xor() => !isValid ? LogicValue.x :
    _value == LogicValue.zero ? _value :
    length.isOdd ? LogicValue.one :
    LogicValue.zero;

  @override
  LogicValues _shiftLeft(int shamt) => 
    LogicValues.fromString(_value.toString() * (length - shamt) + '0' * shamt);

  @override
  LogicValues _shiftRight(int shamt) =>
    LogicValues.fromString( '0' * shamt + _value.toString() * (length - shamt));

  @override
  LogicValues _shiftArithmeticRight(int shamt) =>
    LogicValues.fromString( (_value == LogicValue.one ? '1' : '0') * shamt + _value.toString() * (length - shamt));
...
abstract class LogicValues {
  ...
  static LogicValues fromInt(int value, int length) => _SmallLogicValues(value, 0, length);

  /// Converts `BigInt` [value] to a valid [LogicValues] with [length] number of bits.
  static LogicValues fromBigInt(BigInt value, int length) => _BigLogicValues(value, BigInt.zero, length);

  /// Constructs a [LogicValues] with the [length] number of bits, where every bit has the same value of [fill].
  static LogicValues filled(int length, LogicValue fill) => _FilledLogicValues(fill, length);
  ...
  static bool _bigIntIsFilled(BigInt x, int length) => (x | _BigLogicValues._maskOfLength(length)) == x;
  ...
  static String _valueString(String stringRepresentation) =>
    stringRepresentation.replaceAllMapped(RegExp('[xz]'),  (m) => m[0] == 'x' ? '0' : '1');
  ...
  static String _invalidString(String stringRepresentation) => 
    stringRepresentation.replaceAllMapped(RegExp('[1xz]'), (m) => m[0] == '1' ? '0' : '1');
  ...
  @override
  String toString() =>
    "$length'b" + List<String>.generate(length, (index) => this[index].toString()).reversed.join();

  /// Returns the `i`th bit of this [LogicValues]
  LogicValue operator[](int index) {
    if(index >= length || index < 0) {
      throw IndexError(index, this, 'LogicValuesIndexOutOfRange', 'Index out of range: $index.', length);
    }
  ...
  LogicValue _bitsToLogicValue(bool bitValue, bool bitInvalid) => 
    bitInvalid ?
  ...
  LogicValues operator&(LogicValues other) => _twoInputBitwiseOp(other, (a, b) => a._and2(b));

  /// Bitwise OR operation.
  LogicValues operator|(LogicValues other) => _twoInputBitwiseOp(other, (a, b) => a._or2(b));

  /// Bitwise XOR operation.
  LogicValues operator^(LogicValues other) => _twoInputBitwiseOp(other, (a, b) => a._xor2(b));
  ...
  LogicValues _twoInputBitwiseOp(LogicValues other, LogicValues Function(LogicValues, LogicValues) op) {
    if(length != other.length) throw Exception('Lengths must match');
    if(other is _FilledLogicValues && this is! _FilledLogicValues) return op(other, this);
  ...
  LogicValues _doMath(dynamic other, dynamic Function(dynamic a, dynamic b) op) {
    if(!(other is int || other is LogicValues || other is BigInt)) throw Exception('Improper argument ${other.runtimeType}');
    if(other is LogicValues && other.length != length) throw Exception('Lengths must match');

    if(!isValid) return LogicValues.filled(length, LogicValue.x);
    if(other is LogicValues && !other.isValid) return LogicValues.filled(other.length, LogicValue.x);

    if(this is _BigLogicValues || other is BigInt || other is _BigLogicValues) {
      var a = toBigInt();
      var b = other is BigInt ? other : 
        other is int ? BigInt.from(other) :
        other is LogicValues ? other.toBigInt() :
        throw Exception('Unexpected big type.');
      return LogicValues.fromBigInt(op(a, b), length);
    } else {
      var a = toInt();
      var b = other is int ? other :
        (other as LogicValues).toInt();
  ...
  LogicValue _doCompare(dynamic other, bool Function(dynamic a, dynamic b) op) {
    if(!(other is int || other is LogicValues || other is BigInt)) throw Exception('Improper arguments ${other.runtimeType}.');
    if(other is LogicValues && other.length != length) throw Exception('Lengths must match');
    ...
    if(this is _BigLogicValues || other is BigInt || other is _BigLogicValues) {
      a = toBigInt();
      b = other is BigInt ? other : 
        other is int ? BigInt.from(other) :
        other is LogicValues ? other.toBigInt() :
        throw Exception('Unexpected big type.');
    } else {
      a = toInt();
      b = other is int ? other :
        (other as LogicValues).toInt();
    }
  ...
  LogicValues operator>>(dynamic shamt) => _shift(shamt, _ShiftType.arithmeticRight);
  ...
  LogicValues _shift(dynamic shamt, _ShiftType direction) {
    ...
    } else { //if(direction == ShiftType.ArithmeticRight) {
