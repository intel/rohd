...
class LogicValue {
  ...
  @override bool operator ==(Object other) => other is LogicValue && other._value == _value;

  @override int get hashCode => _value.hashCode;

  @override
  String toString() {
    return this == LogicValue.x ? 'x' :
           this == LogicValue.z ? 'z' :
           this == LogicValue.one ? '1' : '0';
  }

  /// Logical inversion.  Returns `x` if invalid.
  LogicValue operator~() {
    if(!isValid) return LogicValue.x;
    return this == LogicValue.zero ? 
      LogicValue.one : LogicValue.zero;
  }

  /// Logical AND operation.
  LogicValue operator&(LogicValue other) {
    if(this == LogicValue.zero || other == LogicValue.zero) return LogicValue.zero;
    if(!isValid || !other.isValid) return LogicValue.x;
    return (this == LogicValue.one && other == LogicValue.one) ? LogicValue.one : LogicValue.zero;
  }

  /// Logical OR operation.
  LogicValue operator|(LogicValue other) {
    return (this == LogicValue.one || other == LogicValue.one) ? LogicValue.one : 
           (isValid && other.isValid)                          ? LogicValue.zero :
           LogicValue.x;
  }
  ...
  static bool isPosedge(LogicValue previousValue, LogicValue newValue, {ignoreInvalid=false}) {
    if(!ignoreInvalid && (!previousValue.isValid | !newValue.isValid)) throw Exception('Edge detection on invalid value');
  ...
  /// thrown, unless [ignoreInvalid] is set to `true`.
  static bool isNegedge(LogicValue previousValue, LogicValue newValue, {ignoreInvalid=false}) {
    if(!ignoreInvalid && (!previousValue.isValid | !newValue.isValid)) throw Exception('Edge detection on invalid value');
