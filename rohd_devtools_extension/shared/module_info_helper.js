...
//
// Used by both rohd-schematic-viewer (plain JS) and rohd-wave-viewer (TypeScript)
// VS Code extensions to resolve format availability (ROHD, SV) for a module.
//
// 2026 May
...
async function lookupSignalFrames(flcPath, moduleName, signalName, format, output) {
  try {
    ...
    const filteredFrames = format
      ? typedFrames.filter(f => (f?.type ?? 'rohd') === format)
      : typedFrames;
