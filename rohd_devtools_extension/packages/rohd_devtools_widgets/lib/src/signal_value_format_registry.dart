...
class SignalValueFormatRegistry {
  ...
  static final RegExp _hexadecimalMarker = RegExp('[a-fxz]');
  ...
  static void update(Iterable<SignalValueFormatPreference> preferences) {
    final replacement = OccurrenceTrie<SignalValueFormat>();
    for (final preference in preferences) {
      _validateSignalAddress(preference.address);
      replacement[preference.address] = preference.format;
    }
    _formatTrie = replacement;
    _notifyListeners();
  }
  ...
  static void setFormatFor(
    ...
    final requestedAddresses = addresses.toList(growable: false)
      ..forEach(_validateSignalAddress);
  ...
  static SignalValueFormat formatForAny(
    ...
    for (final address in addresses) {
      ...
      _validateSignalAddress(address);
  ...
  /// Increments the registry change generation after a successful mutation.
  static void _notifyListeners() => _changes.value++;

  /// Ensures [address] identifies a signal rather than an occurrence.
  static void _validateSignalAddress(OccurrenceAddress address) {
  ...
  /// Returns whether [value] contains unknown (`x` or `z`) digits.
  ...
  /// Returns [value] in its canonical waveform representation when possible.
