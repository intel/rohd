...
//
// web_vm_connection_strategy.dart
// VM connection strategy for web platforms.
// Uses web_socket_channel for browser-compatible WebSocket connection.
//
// 2026 January
...
class _WebLog extends Log {
  final Logger _logger = Logger('VMService.Web');
  @override
...
/// VM connection strategy for web platforms (browser).
/// Uses web_socket_channel instead of dart:io WebSocket.
class WebVmConnectionStrategy extends VmConnectionStrategy {
  void _logWebSocketError(Object error) {
    Logger('VMService.Web').severe('WebSocket error: $error');
  }
  ...
  Future<VmConnectionResult> connect(String uri) async {
    ...
    final wsUrl = normalizedUri.toString();
    final channel = WebSocketChannel.connect(Uri.parse(wsUrl));
    final socketSink = channel.sink;

    // Wait for the connection to be established.
    await channel.ready.timeout(
      const Duration(seconds: 10),
      onTimeout: () =>
          throw TimeoutException('WebSocket connection timed out after 10 s'),
    );
