...
Next, we can see how extending your `Logic` to `Module` enables the generation of system Verilog code. Building on the previous example, we've made some slight modifications by adding `simModule.build()` and `SystemVerilogService(simModule, register: false).synthOutput`.
...
void main() async {
    ...
    print(SystemVerilogService(simModule, register: false).synthOutput);
    ...
    test(
        ...
        () => expect(SystemVerilogService(simModule, register: false).synthOutput, contains('module SimpleModule(')));
}
```
